...
// const simplifiedData = data.map(({name, gender}) => ({name, gender}))
...
document.querySelector('form').addEventListener('submit', (e) => {
  ...
  const renderTableRows = json => json.map(({ name, gender }) => `<tr>
    <td>${name}</td>
    <td>${gender}</td>
  </tr>
  `).join('')

  const calcTotalGenderNum = (json, gender) => json.reduce((acc, person) => {
    if (gender === person.gender) { acc++ }
    return acc
  }, 0)
  ...
  fetch(document.querySelector('#url').value)
    ...
    .then(json => {
      root.innerHTML = `<table>
        <thead>
          <tr><td colspan="2">People Data</td></tr>
        </thead>
        <tbody>
          <tr class="first-row">
            <td>Name</td>
        ...
        <tfoot>
          <tr>
            <td>${calcTotalGenderNum(json, 'Female')} Female</td>
            <td>${calcTotalGenderNum(json, 'Male')} Male</td>
          </tr>
        </tfoot>
      </table>
      `
